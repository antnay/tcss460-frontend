...
const sampleTVShows: Omit<TVShow, "userId">[] = [
  {
    ...
    poster:
      "https://images.unsplash.com/photo-1574267432644-f610a51acebd?w=400&h=600&fit=crop",
  },
  {
    id: "2",
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!storedMovies) {
      ...
      const initialMovies = sampleMovies.map((m) => ({
        ...m,
        userId: "sample",
      }));
    ...
    if (!storedTVShows) {
      // Initialize with sample data
      const initialTVShows = sampleTVShows.map((t) => ({
        ...t,
        userId: "sample",
      }));
